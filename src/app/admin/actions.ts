...
function imageFiles(formData: FormData, fieldName: string) {
  return formData
    .getAll(fieldName)
...
async function productImageUrls(formData: FormData) {
  const candidates = [...new Set(formData.getAll("newImageUrls").map(String))]
    .filter((url) => filenameFromUploadUrl(url));
  const checks = await Promise.all(
    candidates.map(async (url) => ({ url, exists: await storedUploadExists(url) })),
  );
  return checks.filter((item) => item.exists).map((item) => item.url);
}

async function uploadIsReferenced(url: string) {
  const [productImage, categories, brands, banners] = await Promise.all([
    db.productImage.findFirst({ where: { url } }),
  ...
  return Boolean(
    productImage
      || categories.some((category: { image: string }) => category.image === url)
      || brands.some((brand: { logo: string }) => brand.logo === url)
      || banners.some((banner: { image: string }) => banner.image === url),
  );
}

async function removeUploadIfUnused(url: string) {
  if (!url.startsWith("/uploads/")) return;
  if (await uploadIsReferenced(url)) return;
...
async function uniqueProductSlug(base: string, currentId?: string) {
  ...
  while (true) {
    const existing = await db.product.findFirst({
      where: { slug: candidate, ...(currentId ? { id: { not: currentId } } : {}) },
      select: { id: true }
    });
    if (!existing) break;
...
async function uniqueCategorySlug(base: string, currentId?: string) {
  ...
  let index = 2;
  while (await db.category.findFirst({ where: { slug: candidate, ...(currentId ? { id: { not: currentId } } : {}) }, select: { id: true } })) {
    candidate = `${base}-${index++}`;
  }
  return candidate;
}
...
async function uniqueBrandSlug(base: string, currentId?: string) {
  let candidate = base || `brand-${Date.now()}`;
  let index = 2;
  while (await db.brand.findFirst({ where: { slug: candidate, ...(currentId ? { id: { not: currentId } } : {}) }, select: { id: true } })) {
    candidate = `${base}-${index++}`;
  }
  return candidate;
}
  ...
  try {
    ...
    await db.product.create({
      data: {
        name,
        slug,
        sku: str(formData, "sku") || `ES-${Date.now()}-${randomUUID().slice(0, 8)}`,
        description: str(formData, "description"),
        price: int(formData, "price"),
        compareAtPrice: str(formData, "compareAtPrice") ? int(formData, "compareAtPrice") : null,
        ...
        images: {
          create: uploaded.map((url, index) => ({ url, alt: name, sortOrder: index })),
        },
      },
    });
  } catch (error) {
...
export async function updateProduct(formData: FormData) {
  ...
  try {
    ...
    const removed = currentImages.filter((image) => removeIds.includes(image.id));
    removedUrls = [...new Set(removed.map((image) => image.url))];

    const remaining = currentImages.filter((image) => !removeIds.includes(image.id));
    const remainingUrls = new Set(remaining.map((image) => image.url));
    const uploaded = await productImageUrls(formData);
    const newUploads = uploaded.filter((url) => !remainingUrls.has(url));

    nextSlug = await uniqueProductSlug(slugify(str(formData, "slug") || name), id);
    ...
    await db.$transaction(async (tx: typeof db) => {
      ...
      await tx.product.update({
        ...
        data: {
          name,
          slug: nextSlug,
          sku: str(formData, "sku") || current.sku,
          description: str(formData, "description"),
          price: int(formData, "price"),
          compareAtPrice: str(formData, "compareAtPrice") ? int(formData, "compareAtPrice") : null,
          currency: str(formData, "currency") || "RUB",
          ...
          brandId: str(formData, "brandId"),
          images: {
            create: newUploads.map((url, index) => ({
              url,
              alt: name,
  ...
  await Promise.allSettled(removedUrls.map(removeUploadIfUnused));
  revalidatePath("/");
  revalidatePath("/catalog");
  if (previousSlug) revalidatePath(`/product/${previousSlug}`);
  if (nextSlug && nextSlug !== previousSlug) revalidatePath(`/product/${nextSlug}`);
...
export async function deleteProduct(formData: FormData) {
  ...
  if (product) {
    const imageUrls = [...new Set((product.images as ProductImage[]).map((image) => image.url))];
    await db.product.delete({ where: { id } });
    await Promise.allSettled(imageUrls.map(removeUploadIfUnused));
  }
...
export async function createBrand(formData: FormData) {
  await requireAdmin();
  const name = str(formData, "name");
  const [uploaded] = await saveImages(imageFiles(formData, "logoFile"));
...
export async function deleteBrand(formData: FormData) {
  await requireAdmin();
  await db.brand.delete({ where: { id: str(formData, "id") } });
...
export async function createReview(formData: FormData) {
  await requireAdmin();
  await db.review.create({ data: { author: str(formData, "author"), city: str(formData, "city"), rating: Math.min(5, Math.max(1, int(formData, "rating", 5))), text: str(formData, "text"), isApproved: bool(formData, "isApproved") } });
  revalidatePath("/");
...
export async function toggleReview(formData: FormData) {
  ...
  if (review) await db.review.update({ where: { id }, data: { isApproved: !review.isApproved } });
  revalidatePath("/");
  redirect("/admin/reviews?saved=1");
}
